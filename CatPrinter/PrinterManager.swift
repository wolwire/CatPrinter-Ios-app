...
final class PrinterManager: NSObject, ObservableObject {
  ...
  func startScan() {
    // Clear previous discoveries and begin scanning. If Bluetooth isn't powered on yet,
    // mark scanningRequested so centralManagerDidUpdateState will start scanning when ready.
    discoveredPrinters = []
    scanningRequested = true
    if central.state == .poweredOn {
      // Scan for all peripherals (nil) to increase discovery coverage; filter in didDiscover
  ...
  func connectToPeripheral(_ peripheral: CBPeripheral) async throws {
    ...
    try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
      ...
      // connectContinuation will be resumed when characteristics discovered
    }
    // Mark connected
...
extension PrinterManager: CBCentralManagerDelegate, CBPeripheralDelegate {
  ...
  func centralManager(
    ...
    // Parse advertisement data to create a short summary useful for debugging
    var advParts: [String] = []
    if let localName = advertisementData[CBAdvertisementDataLocalNameKey] as? String {
      advParts.append("name:\(localName)")
    }
    if let svcs = advertisementData[CBAdvertisementDataServiceUUIDsKey] as? [CBUUID], !svcs.isEmpty
    {
      let svcStrings = svcs.map { $0.uuidString }.joined(separator: ",")
      advParts.append("svcs:\(svcStrings)")
    }
    if let manuf = advertisementData[CBAdvertisementDataManufacturerDataKey] as? Data {
      advParts.append("mfg:0x\(manuf.map { String(format: "%02X", $0) }.joined())")
    }
    if let txPower = advertisementData[CBAdvertisementDataTxPowerLevelKey] as? Int {
      advParts.append("tx:\(txPower)")
    }
    let advSummary = advParts.isEmpty ? nil : advParts.joined(separator: " ")

    let advName = advertisementData[CBAdvertisementDataLocalNameKey] as? String
    let name = peripheral.name ?? advName ?? peripheral.identifier.uuidString

    print(
      "Discovered: \(name) (id=\(peripheral.identifier)) rssi=\(RSSI) adv=[\(advSummary ?? "-")] fullAdv=\(advertisementData)"
    )
    ...
    DispatchQueue.main.async {
      if !self.discoveredPrinters.contains(where: { $0.id == peripheral.identifier }) {
        self.discoveredPrinters.append(
          DiscoveredPrinter(
            id: peripheral.identifier, name: name, advSummary: advSummary, peripheral: peripheral))
      }
    }
  }
